...
StaticShock.prototype.buildFile = function(file) {
    ...
    var relativePath = path.relative(this._options.relative, file),
    ...
    if(controllerFunction) {
        if(this._buildFileTree.length <= 1) {
    ...
    if(list) {
        ...
        for(var i = 0; i < len; i++) {
            ...
            try {
                ...
                if(stat.isFile()) {
                    if(this._controller.PreBuildFile) {
                        this._controller.PreBuildFile(filepath);
                    }
                    var data = this.buildFile(filepath);
                    if(this._controller.PostBuildFile) {
                        this._controller.PostBuildFile(filepath);
                    }
